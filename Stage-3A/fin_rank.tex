...
%TODO We only used that f is in L2, and not that it is in H3
The compact operators that we are studying are of the form: \[K(f)(x) = \int K(x,y)f(y)\mathrm{d}y \]
Assuming that we can bound the growth of the kernel at infinity, a natural choice of finite rank approximation on a bounded domain $[-A,A]$ would be to use a Riemann sum : \[K_n(f)(x) := \sum_{i=1}^n \delta K(x,y_i)f(y_i)\]
...
\begin{eqnarray*}
K(f)(x) &=& \int K(x,y)f(y)\mathrm{d}y = \int \hat{K}(x,\xi)\hat{f}(\xi)\mathrm{d}\xi \\
&=& \int \hat{f}(\xi) \int K(x,y)e^{-2i\pi\xi y}\mathrm{d}y \mathrm{d}\xi 
\end{eqnarray*}
