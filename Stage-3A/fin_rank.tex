...
%TODO check si il manque pas une conjuguaison dans ma def du prod L^2 pour appliquer parseval
...
\end{eqnarray*}
Defining $\mu_1(\xi):= - \frac{4\pi^2 \xi^2}{(1+\xi^2)^3 }$, $\mu_2(\xi) := - \frac{i16\pi^3\xi^3}{(1+\xi^2)^3 }$ and $\mu_3(\xi) :=  \frac{16\pi^4\xi^4}{(1+\xi^2)^3 }$ , we have that 
\[M_2z :=  \left(\mathcal{F}^{-1}(\mu_1)*\varphi_2''z\right)+\left(\mathcal{F}^{-1}(\mu_2)*\varphi_2'z\right)+\left(\mathcal{F}^{-1}(\mu_3)*\varphi_2z\right) \]
satisfies \fcref{c2}.
\end{document}
