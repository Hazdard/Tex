...
~\\
Then for the second part, by Fubini's property:
\begin{eqnarray*}
...
&=& \left| \int_{\R \setminus [-A,A]} \lambda(x-y)\varphi(y) f(y)\mathrm{d}y \right| = \left| \int_{\R \setminus [x-A,x+A]} \lambda(u)\varphi(x-u) f(x-u)\mathrm{d}u \right| 
\end{eqnarray*}
...
\end{eqnarray*}
Since we can show that $\lambda(x)=\frac{\pi}{4}e^{-2\pi \lvert x\rvert}\left(2\pi^2x^2+3\pi \lvert x\rvert +\frac{3}{2}\right)$,
