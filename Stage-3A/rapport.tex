...
% Le résultat qu'on veut montrer est beaucoup plus fort que simplement cette equation a une donnée initiale bizarre qui engendre une singularité. On prouve qu'il y a un ouvert de données initiales qui génèrent TOUTES la MÊME singularité vue asymptotiquement via le changement de variable auto-similaire ! Ce n'est donc pas un phénomène illusoire, négligeable (cf tr\begin{equation}
•
\end{equation}iadique de cantor pour faire péter Euler), et de plus Tristan semble penser que le comportement pour Burgers est universel, quitte a bien changer les variables. % Deux trucs dans le papier 1 : géométrie de la singularité stable (au sens C^4 pour la géométrie asymptotique auto similaire du choc) ; et purement azymuthale (en se ramenant à la sol 1D) ; et viscosite non nulle et bornee.
% Citer Chen/Hou blow up cylindrical boundary for smooth inital data.
% Interet de self-smilarity : universallité, unifie. Et deja présent chez Leray ?
% On veut du stable car on fait du numerique et donc si instable alors les erreurs numeriques nous emmène loin. (En fait ps sur, gros trouver une vrai def)
...
% DIRE QUE LES MODES QU'IL RESTE À LA FIN SONT TRAITÉS DANS LA PARTIE NON LINÉAIRE, EN CHOISISSANT BIEN LA DONNÉE INITIALE.
