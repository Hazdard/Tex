...
\begin{theorem}
Avec une probabilité $\geq 1 - \frac{n+d-1}{q}^{\frac{td}{n+2d}}$, on a que $\text{vol}(\mathcal{L}_2)=q^{td-(n+d-1)}$.\\ Si on utilise les ordres de grandeurs proposés dans le schéma de chiffrement de \cite{mplwe}, on a : 
\[\mathbb{P}(\text{vol}(\mathcal{L}_2)=q^{td-(n+d-1)}) \geq 1 - (\frac{1}{n^\frac{3}{2}\sqrt{\log{n}}})^{\log{n}}\]
\end{theorem}
\newpage
\textbf{Preuve :} Il suffit de montrer qu'avec bonne probabilité, $A_2:=[\text{Toep}^{d,n}(a'_i)_{i\leq t}]$ est de rang plein. \\
On commence par rappeller de lemme de Schwartz-Zippel : Pour un polynôme multivarié de degré $n$, à coefficients dans $\mathbb{Z}/p\mathbb{Z}$, la probabilité d'annuler le polynôme en choisissant les variables uniformément est au plus $\frac{n}{p}$. \\
On considère donc la matrice carré constituée des $n+d-1$ premières lignes de $A_2$, que l'on note $A_2(1)$. Le déterminant de cette sous-matrice est un polynôme à plusieurs variables, de degré $n+d-1$, dont les variables sont choisies uniforméments dans $\mathbb{Z}_q$. Alors, d'après le lemme , on a que : \\ $\mathbb{P}[\text{det}(A_2)=0] \leq \frac{n+d-1}{q}$
