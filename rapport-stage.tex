...
\begin{itemize}
\item Le réseau initial $\mathcal{L}_0 = \{ [(\text{Rot}_f(a_i))_{i\leq t}]\cdot x \text{ mod }q, \ x\in \mathbb{Z}^m \}$
\item Le réseau intermédiaire $\mathcal{L}_1 = \{ [(\text{Rot}_f^d(a_i))_{i\leq t}]\cdot x \text{ mod }q, \ x\in \mathbb{Z}^d \}$  qui correspond au projeté en dimension $d$ du réseau initial
\item Le réseau final $\mathcal{L}_2 = \{ [(\text{Toep}^{d,n}(a'_i))_{i\leq t}]\cdot x \text{ mod }q, \ x\in \mathbb{Z}^d \}$
\end{itemize}
