...
\begin{thm}[Injection de Sobolev]\label{inject}
...
Ensuite, comme $\rho >0$, le \fcref{inject} donne $u\in L^\infty(\R^d)$. Par définition, on a \[T_{F'(u)}u = S_{-3}g\ u_0 + \sum_{k=0}^\infty S_{k-2} g \Delta_k u \]
...
Cela découle du \fcref{meyer}, que l'on peut appliquer d'une part grâce à (\ref{ineq_l2}), et d'autre part car on a l'inégalité :
