...
\begin{itemize}
\item[•]Soit $\alpha >0$, si $\sup 2^{k\alpha}\|u\|_{L^\infty} < +\infty$, alors $u=\sum u_k \in C^\alpha_*(\R^d)$, et 
\begin{equation*}
...
\begin{proof}
Par hypothèse sur le support des $(u_k)_{k\geq -1}$, pour tout $q \geq -1$,  en notant $N := \lfloor q-1-\log_2(R) \rfloor$, on a: 
\begin{equation*}
\Delta_q u = \sum_{k\geq q-N} \Delta_qu_k
\end{equation*}
On en déduit :
\begin{equation*}
2^{q\alpha}\|\Delta_qu\|_{L^\infty}\leq  2^{q\alpha}\sum_{k\geq q-N}\|\Delta_qu_k\|_{L^\_infty} \leq 2^{q\alpha}\sum_{k\geq q-N}\|u_k\|_{L^\infty} \leq \sum_{k\geq q-N} 2^{k\alpha}\|u_k\|_{L^\infty} 2^{(q-k)\alpha} \leq \sum_{k\in \Z} a_k b_{q-k}
\end{equation*}
où :
\begin{itemize}
...
\end{equation*}
D'autre part, en utilisant l'inégalité de Bernstein \eqref{bernstein}, 
\end{proof}
